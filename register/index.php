...
if(isset($_POST['pw']) && isset($_POST['user']) && isset($_POST['email'])){ //Registering New Password
	...
	if($x == 0){
		...
		if($x == 0){
			$p = $row['memo'];
			$url = "https://steemscan.com/?user=steemauto.app";
		...
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				if(y['reg'] == 1){
					...
					document.getElementById('result').innerHTML = '<div class="alert alert-info">Please Send <code>0.001 SBD</code> to Account <code>@steemauto.app</code> with memo <code>'+y["memo"]+'</code> Then Try again.</div>';
				}else if(y['reg'] == 3){
